...
const TeacherDashboard = () => {
  ...
  const [electiveBatches, setElectiveBatches] = useState([]);
  ...
  const fetchHubData = useCallback(async () => {
    try {
      setLoading(true);
      const [timeRes, userRes, batchRes] = await Promise.all([
        API.get('/timetable'),
        API.get('/auth/users'),
        API.get('/batches')
      ]);
      ...
      const markedClasses = myClasses.map(c => ({
        ...c,
        isElective: electiveBatchNames.includes(c.batch)
      }));
      setTimetable(markedClasses);
                          ...
                          {timeSlots.map((slot, i) => {
                            ...
                            return (
                              ...
                              >
                                {session ? (
                                  <>
                                    <p className={`text-[9px] font-bold uppercase truncate w-full leading-tight ${isLive ? 'text-white' : `text-${baseColor}-400`}`}>
                                      {session.subject}
                                    </p>
                                    <p className={`text-[7px] truncate w-full ${isLive ? 'text-white/80' : `text-${baseColor}-300`}`}>
                                      {session.subjectCode}
                                    </p>
                                    <div className="flex items-center gap-1 mt-1">
                                      ...
                                      <div className={`px-1.5 py-0.5 rounded text-[7px] font-bold ${
                                        isLive ? 'bg-white text-red-600' : `bg-${baseColor}-500 text-white`
                                      }`}>
                                        R-{session.room}
                                      </div>
                                    </div>
                                    {session.batch && (
                                      <p className={`text-[6px] mt-0.5 ${isLive ? 'text-white/70' : 'text-slate-400'}`}>
                                        {session.batch}
                                      </p>
                                    )}
                                  </>
                                ...
                                {session ? (
                                  <>
                                    <p className={`text-[9px] font-bold uppercase truncate leading-tight ${
                                      isLive ? 'text-white' : `text-${baseColor}-400`
                                    }`}>
                                      {session.subject}
                                    </p>
                                    <p className={`text-[7px] truncate ${
                                      isLive ? 'text-white/80' : `text-${baseColor}-300`
                                    }`}>
                                      {session.subjectCode}
                                    </p>
                                    <div className="flex items-center gap-1 mt-1">
