      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'matrix' && (
          <motion.div key="matrix" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} id="faculty-matrix-capture" className="bg-white rounded-[2rem] md:rounded-[4rem] p-4 md:p-12 shadow-2xl border border-slate-100 flex flex-col overflow-hidden relative text-left">
             ...
             {/* 🚀 START OF UPDATED ZOOMABLE & DRAGGABLE MATRIX */}
<div className="w-full overflow-hidden border border-white/5 rounded-[2rem] bg-black/10 cursor-grab active:cursor-grabbing">
  <motion.div 
    ...
    className="w-full"
  >
    <div className="grid grid-cols-7 gap-1 md:gap-5 min-w-[900px] md:min-w-full py-6 md:py-10 text-center">
        
        {/* TIME COLUMN */}
        <div className="col-span-1 space-y-2 md:space-y-4 pt-10 md:pt-24 text-right pr-2 border-r border-white/5">
            {timeSlots.map(s => (
              <div key={s} className="h-12 md:h-20 flex items-center justify-end text-[6px] md:text-[11px] font-black text-slate-400 uppercase italic tracking-tighter pr-2">
                {s.split(' - ')[0]}
              </div>
            ))}
        </div>
        ...
        {days.map(day => (
            <div key={day} className="col-span-1 space-y-2 md:space-y-4">
                <h1 className={`text-[7px] md:text-xs font-black uppercase tracking-[0.1em] pb-4 md:pb-10 border-b border-white/5 mb-4 md:mb-8 pr-4 ${day === currentDay ? 'text-orange-500 underline underline-offset-[10px] decoration-2' : 'text-slate-400'}`}>
                  {day.slice(0, 3)}
                </h1>
                ...
                {timeSlots.map((slot, i) => {
                    ...
                    return (
                        <div key={i} className={`h-12 md:h-20 rounded-md md:rounded-[2rem] border flex flex-col items-center justify-center p-0.5 md:p-4 transition-all duration-500 ${s ? (isLive ? 'bg-orange-600 border-white shadow-[0_0_20px_rgba(234,88,12,0.6)] z-20 scale-105 text-white' : 'bg-orange-50 border-orange-200 opacity-100') : 'bg-slate-50 border-transparent opacity-5'}`}>
                            {s ? (
                              <>
                                {/* pr-4 added to fix italic clipping */}
                                <p className={`text-[5px] md:text-[10px] font-black uppercase italic truncate w-full text-center leading-none pr-4 ${isLive ? 'text-white' : 'text-orange-600'}`}>
                                  {s.subject}
                                </p>
                                <div className={`mt-0.5 md:mt-2 px-1 py-0 rounded-[2px] md:rounded text-[4px] md:text-[8px] font-black ${isLive ? 'bg-white text-orange-600' : 'bg-orange-600 text-white'} pr-1`}>
                                  {s.batch || `R-${s.room}`}
                                </div>
                              </>
                            ) : (
                              <span className="text-[5px] opacity-10 italic pr-4 uppercase">Void</span>
                            )}
                        </div>
                    )
                })}
